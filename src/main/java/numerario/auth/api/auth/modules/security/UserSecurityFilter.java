...
import numerario.auth.api.auth.modules.providers.UserJWTProvider;
...
public class UserSecurityFilter extends OncePerRequestFilter {
    ...
    public UserSecurityFilter(UserJWTProvider userJWTProvider) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (request.getRequestURI().startsWith("/user")) {
            ...
            if (header != null) {
                var decodedJWT = this.userJWTProvider.validateToken(header);
